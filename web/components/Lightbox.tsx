...
export default function Lightbox({ images }: { images: { url: string; isCover?: boolean }[] }) {
  ...
  const resolvedImages = images?.map((image) => ({ ...image, url: resolveMediaUrl(image.url) })) || [];
        ...
        {resolvedImages.slice(0, 4).map((img, idx) => (
          <div key={img.url} className="relative aspect-[4/3] rounded-xl overflow-hidden cursor-pointer">
            ...
            {idx === 3 && resolvedImages.length > 4 && (
              <div className="absolute inset-0 bg-black/50 text-white flex items-center justify-center text-sm font-semibold">+{resolvedImages.length - 3} more</div>
            )}
          </div>
        ))}
      </div>
      {open && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center px-4" onClick={() => setOpen(false)}>
          <div className="relative w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
            <Image src={resolvedImages[active].url} alt="large" width={1600} height={900} className="w-full h-auto rounded-2xl object-contain" />
